...
export default class GetAttractionsByPlaceId {

	constructor (readonly standAppGateway: StandAppGateway) {

	}

	async handle(placeId: string): Promise<any> {

		const attractions = await this.standAppGateway.getAttractionsByPlaceId(placeId);
